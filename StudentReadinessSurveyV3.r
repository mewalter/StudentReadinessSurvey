...
name_vec3 = data.frame("vars"=names_vars3,"labels"=names_labels3)

scale <- c("Strongly disagree", "Disagree", "Slightly Disagree", "Slightly Agree", "Agree", "Strongly agree")

filename <- paste0(DataDir,"ICS53Sp24 Survey1.csv")
alldata <- read.csv(filename, col.names = names_vars3) 
alldata$class <- "ICS53Sp24_1" 

transfercounts <- alldata %>% dplyr::count(transfer, name = 'N') 
...
# Survey results put into factors
factordata <- alldata %>% mutate_at(vars(starts_with("X")), funs(factor(., levels = scale, ordered = TRUE))) %>% 
  rename_at(vars(name_vec3$vars), ~ name_vec3$labels)


# %>% mutate(across('class',str_replace,'EngMAE151A','EngMAE151A1')) 
...
# All Survey 
p <- likert(factordata[,c(5:19)])
plot(p, legend.position="top", wrap=90, ordered=FALSE, group.order=names_labels3[5:19]) 

# Look at transfer vesus non-transfer
p <- likert(factordata[,c(5:19)], grouping = factordata[,3])
...
# old time to complete plot ... need to generate this data with columns 1 and 4.
ggplot(factordata, aes(x=factor(0), y=completet)) + 
